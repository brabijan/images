...
class ImagesExtension extends Nette\Config\CompilerExtension {
	...
	public function loadConfiguration() {
		...
		$install = 'Brabijan\Images\Latte\ImagesMacro::install';
		$engine->addSetup($install.'(?->compiler)', array('@self'));

		$builder->addDefinition($this->prefix('imagePipe'))->setClass('Brabijan\Images\ImagePipe', array($config["assetsDir"], $this->containerBuilder->parameters["wwwDir"]))
